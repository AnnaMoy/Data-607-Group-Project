...
#remove line 1 which initally has the old titles
moviedf <- df[-1,]
...
#filter dataset for movies of 2019 and 2020
...
# filter for 2019 movies and calculated the profit (total gross - budget)
movie2019_numeric_sorted <- movie2020_numeric %>%
  filter(year == 2019) %>%
  mutate(profit = (total_gross_numeric- budget)) %>%
  select(name, budget, total_gross, profit)
  
# Top 5 movies in 2019
top_5_profitable_2019 <- head(movie2019_numeric_sorted, 5)
top_5_profitable_2019
```

From listing the top 5 movies, we can see that Star Wars: Episode IX - The Rise of Skywalker was the most successful movie in 2019.
...
#T Test 
t.test(profit ~ year, data = gross2020)
...
ggplot(gross2020, aes(yearchar, profit)) +
  geom_boxplot() +
  labs(
...
We can see through our analysis that movies had a large impact on views, especially during the pandemic, where it became one of the main source of entertainment. The pandemic definitely had an impact on the profits in movies in 2020 as they were significantly lower than those in 2019. [add more?]
