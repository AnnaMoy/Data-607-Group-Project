...
As the movie and entertainment industry have a large impact on today’s pop culture, we’re intrigued to see how successful certain movies were over the years, especially in 2020. We will also be delving into the gross income of a variety of movies in order to find which movie was the most profitable. 
...
Which movie in 2020 was the most successful?
...
ggplot(movie2020_numeric, aes(x = budget, y = total_gross_numeric)) +
  geom_point() +
  labs(title = "Budget vs Gross Income for Movies (2020)",
       x = "Budget (in million $)",
       y = "Gross Income (in million $)") +
  theme_minimal()
```

Using a scatterplot to compare budget against gross_income of each movie, we can see a general positive relation, however, there are a good amount of movies that had a large budget but did not make revenue.
...
From this visual we can identify which movies had a total gross that surpassed their budget, thus making a profit. We can also see that a majority of movies did make some profit.
...
top_5_profitable <- head(movie2020_numeric, 5)

top_5_profitable
print(top_5_profitable[, c("name", "budget", "total_gross", "profit")])
```
From listing the top 5 movies, we can see that Star Wars: Episode IX - The Rise of Skywalker was the most successful movie in 2020.
...
We can see through our analysis that movies had a large impact on views, especially during the pandemic, where it became one of the main source of entertainment.[add more?]
